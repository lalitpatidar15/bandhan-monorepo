...
import { BarChart3, Bell, BookOpen, BriefcaseBusiness, Building2, ChevronDown, ClipboardList, DollarSign, FileText, GraduationCap, LayoutDashboard, LifeBuoy, LogOut, Package, Scale, Settings, ShieldCheck, ShoppingCart, Tags, TicketPercent, Users, X } from 'lucide-react';
...
export default function Sidebar({ currentPage, setCurrentPage, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return <>
    {mobileOpen ? <button type="button" aria-label="Close admin navigation" className="fixed inset-0 z-40 bg-black/45 backdrop-blur-[2px] lg:hidden" onClick={onMobileClose} /> : null}
    <aside aria-label="Admin navigation" className={`admin-sidebar fixed inset-y-0 left-0 z-50 flex h-dvh w-[17.5rem] shrink-0 flex-col overflow-hidden transition-transform duration-200 lg:static lg:z-auto lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
    <div className="border-b border-white/10 px-5 py-5"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#8B3A28] text-lg font-black text-white shadow-lg shadow-black/30">B</span><div className="min-w-0 flex-1"><p className="font-bold tracking-tight text-white">Bandhan</p><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#f1a596]">Admin control centre</p></div><button type="button" aria-label="Close navigation" onClick={onMobileClose} className="flex h-10 w-10 items-center justify-center rounded-lg text-[#e8d5cc] hover:bg-white/10 hover:text-white lg:hidden"><X className="h-5 w-5" /></button></div></div>
    <nav className="admin-scrollbar flex-1 overflow-y-auto px-3 py-4">{sections.map((section) => { const expanded = open[section.title]; return <div key={section.title} className="mb-3"><button type="button" aria-expanded={expanded} onClick={() => setOpen((previous) => ({ ...previous, [section.title]: !expanded }))} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-[10px] font-bold uppercase tracking-[0.15em] text-[#e8d5cc] hover:bg-white/5 hover:text-white"><span>{section.title}</span><ChevronDown className={`h-3.5 w-3.5 transition-transform ${expanded ? '' : '-rotate-90'}`} /></button>{expanded && <div className="mt-1 space-y-1">{section.items.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => selectPage(id)} className={`admin-sidebar-item w-full ${currentPage === id ? 'admin-sidebar-item-active' : ''}`}><Icon className="h-4 w-4 shrink-0" /><span>{label}</span></button>)}</div>}</div>; })}</nav>
    <div className="border-t border-white/10 p-3"><div className="mb-2 rounded-xl bg-white/5 px-3 py-2.5"><p className="text-xs font-semibold text-white">Administrator</p><p className="mt-0.5 text-[11px] text-[#e8d5cc]">Full platform access</p></div><button type="button" onClick={() => { localStorage.removeItem('adminToken'); localStorage.removeItem('adminUser'); window.location.href = '/admin/login'; }} className="admin-sidebar-item w-full text-rose-300 hover:bg-rose-500/10 hover:text-rose-100"><LogOut className="h-4 w-4" /> Logout</button></div>
  </aside>
  </>;
}
